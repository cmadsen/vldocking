...
public class DockingSelectorDialog extends JDialog {
	...
	private String wizardLabelText = "<HTML><BODY><P>This window helps you to manage the " + " views composing your application workspace.</P>" + "<P><P>By clicking on the table checkboxes below, you can select if " + "a view will be diplayed or not </P>"

	+ " <P><P> Click on the <b>Ok</b> button to apply your changes or on " + "the <b>Cancel</b> button if you do not want to save your changes." + " </BODY></HTML>";
	...
	private void init() {
		...
		wizardLabel.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createMatteBorder(0, 0, 1, 0, Color.DARK_GRAY), BorderFactory.createEmptyBorder(5, 5, 5, 5)));

		getContentPane().add(wizardLabel, BorderLayout.NORTH);

		// ESCAPE == CLOSE
		KeyStroke ESC = KeyStroke.getKeyStroke(KeyEvent.VK_ESCAPE, 0);
		getRootPane().getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT).put(ESC, "CLOSE");
		getRootPane().getActionMap().put("CLOSE", closeAction);
		table.getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT).put(ESC, "CLOSE");
	...
	class DockablesTableModel extends AbstractTableModel {
		...
		DockablesTableModel(DockableState[] states) {
			...
			for(int i = 0; i < states.length; i++) {
				visibleViews.put(states[i], states[i].getLocation() == DockableState.Location.CLOSED ? Boolean.FALSE : Boolean.TRUE);
			}
		}
