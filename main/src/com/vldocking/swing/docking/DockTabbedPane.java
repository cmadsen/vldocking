...
public class DockTabbedPane extends JTabbedPane implements DockDropReceiver, DockableDragSource, TabbedDockableContainer {
		...
		addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if(e.getClickCount() == 2 && SwingUtilities.isLeftMouseButton(e)) {
					...
					if(d != null && d.getDockKey().isMaximizeEnabled() && d.getDockKey().getLocation() == DockableState.Location.DOCKED) { // 2005/10/10 don't maximize floatings
						desktop.maximize(d);
					}
				}
			}
		...
		if(d != null) {
			...
			if(key.getLocation() == DockableState.Location.DOCKED) {
				if(key.isAutoHideEnabled()) {
					popup.add(new JMenuItem(TabbedContainerActions.createHideTabAction(d, desktop)));
					menuCount++;
				}
				if(key.isFloatEnabled()) {
					popup.add(new JMenuItem(TabbedContainerActions.createFloatTabAction(d, desktop)));
					menuCount++;
				}
				if(key.isMaximizeEnabled()) {
					popup.add(new JMenuItem(TabbedContainerActions.createMaximizeTabAction(d, desktop)));
					menuCount++;
				}
				if(key.isCloseEnabled()) {
					JMenuItem mItem = new JMenuItem(TabbedContainerActions.createCloseAction(d, desktop));
	...
	private void addDockableContainer(final SingleDockableContainer dc, int tab) {
		...
		final JTabbedPaneSmartIcon smartIcon = new JTabbedPaneSmartIcon(key.getIcon(), key.getName(), new SmartIconJButton[] {closeIcon});
