...
public class BorderSplitter {
	...
	private void acceptSplit(DockDropEvent event, DockingConstants.Split position) {
		...
		if(container instanceof TabbedDockableContainer) {
			...
			if(tabAncestor != null && tabAncestor.getTabCount() == 0 && tabAncestor == delegator) {//2007//11/15
				...
				event.getDesktop().split(remainingDockable, // the only remaining dockable
				dockable, position);
			} else {
	...
	private void acceptDragSplit(DockDragEvent event, DockingConstants.Split position, Shape shape) {
		...
		if(container instanceof TabbedDockableContainer) {
			event.setDockingAction(new DockingActionSplitComponentEvent(event.getDesktop(), dockable, initialLocation, borderSpliterLocation, container, position, 0.5f));
		} else {
			if(tabAncestor != null && tabAncestor.getTabCount() == 1 && tabAncestor == delegator) {
				...
				// which is done by the standard desktop "split" public method.
				Dockable remainingDockable = tabAncestor.getDockableAt(0);
				event.setDockingAction(new DockingActionSplitDockableEvent(event.getDesktop(), dockable, initialLocation, borderSpliterLocation, remainingDockable, position, 0.5f));
			} else {
				// standard dropping, just use the internal API to relayout the desktop.
				event.setDockingAction(new DockingActionSplitComponentEvent(event.getDesktop(), dockable, initialLocation, borderSpliterLocation, delegator, position, 0.5f));
			}
		}
	...
	private void scanDrop(DockEvent event, boolean drop) {
		...
		if(min == distTop && min < delegator.getHeight() / 4) {
			...
			if(drop) {
				...
				Rectangle2D r2d = new Rectangle2D.Float(0, 0, size.width, bestHeight);
			...
			if(drop) {
				...
				Rectangle2D r2d = new Rectangle2D.Float(0, 0, bestWidth, size.height);
			...
			if(drop) {
				...
				Rectangle2D r2d = new Rectangle2D.Float(0, size.height - bestHeight, //20,
				size.width, bestHeight);
			...
			if(drop) {
				...
				Rectangle2D r2d = new Rectangle2D.Float(size.width - bestWidth, 0, bestWidth, size.height);
