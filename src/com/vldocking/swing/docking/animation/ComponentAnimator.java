...
package com.vldocking.swing.docking.animation;
...
public class ComponentAnimator {
	...
	public void start() {
		if(duration == 0) { // heavy weight == no animation
			...
			comp.setBounds(startBounds.x, startBounds.y, startBounds.width, startBounds.height);
			comp.validate();
	...
	private class AnimationActionListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if(elapsed < duration) {
				...
				newBounds.height = (int) (startBounds.height * f1 + endBounds.height * f2);
				comp.setBounds(newBounds.x, newBounds.y, newBounds.width, newBounds.height);
				comp.validate();
				fireAnimationEvent(new AnimationEvent(comp, AnimationEvent.ANIMATION_FRAME));
			} else {
