...
public class DockingDesktop extends JLayeredPane {
	...
	public DockingDesktop(String desktopName, DockingContext context) {
				...
				if(event.getComponent() == DockingDesktop.this) {
					...
					if(maximizedComponent != null) {
						if(DockingPreferences.isLightWeightUsageEnabled()) {
							maximizedComponent.setBounds(i.left, i.top, w - i.left - i.right, h - i.top - i.bottom);
						} else {
							if(DockingPreferences.isSingleHeavyWeightComponent()) {
								if(DockingUtilities.isHeavyWeightComponent(maximizedComponent)) {
									maximizedComponent.setBounds(i.left, i.top, w - i.left - i.right, h - i.top - i.bottom);
								} else {
									maximizedComponent.getParent().setBounds(i.left, i.top, w - i.left - i.right, h - i.top - i.bottom);
								}
							} else {
								maximizedComponent.getParent().setBounds(i.left, i.top, w - i.left - i.right, h - i.top - i.bottom);
							}
		...
		expandPanel.addPropertyChangeListener(AutoHideExpandPanel.PROPERTY_EXPANDED, new PropertyChangeListener() {
		...
		addAncestorListener(new AncestorListener() {
			...
			public void ancestorAdded(AncestorEvent event) {
				KeyboardFocusManager.getCurrentKeyboardFocusManager().addPropertyChangeListener("focusOwner", focusHandler); //2006/08/21 : back to life (was in comments in 2.1.0)
			...
			public void ancestorRemoved(AncestorEvent event) {// 2005/12/08
				KeyboardFocusManager.getCurrentKeyboardFocusManager().removePropertyChangeListener("focusOwner", focusHandler); //2006/12/19
	...
	private void createTab(Dockable base, Dockable dockable, int order, boolean select, boolean triggerEvents) {
		...
		DockingActionEvent dae = new DockingActionCreateTabEvent(this, dockable, currentLocation, newState.getLocation(), base, order);
		...
		if(baseTab != null) {
			...
			DockingUtilities.replaceChild(((Component) baseOldContainer).getParent(), (Component) baseOldContainer, (Component) baseTab);
	...
	public void split(Dockable base, Dockable newDockable, DockingConstants.Split position, double proportion) {
		...
		DockableContainer dockableContainer = DockableContainerFactory.getFactory().createDockableContainer(newDockable, DockableContainerFactory.ParentType.PARENT_SPLIT_CONTAINER);
		...
		if(baseTab != null) {
			...
			DockingUtilities.replaceChild(((Component) baseTab).getParent(), ((Component) baseTab), split);
			...
			DockableContainer baseContainer = DockingUtilities.findSingleDockableContainer(base);

			assert baseContainer != null;

			DockingUtilities.replaceChild(((Component) baseContainer).getParent(), ((Component) baseContainer), split);
	...
	public void splitComponent(Component base, Dockable dockable, DockingConstants.Split position) {
		...
		DockableStateWillChangeEvent dswe = new DockableStateWillChangeEvent(currentState, newState);
		DockingActionEvent dae = new DockingActionSplitComponentEvent(this, dockable, currentLocation, newState.getLocation(), base, position, 0.5f);
		...
		DockableContainer dockableContainer = DockableContainerFactory.getFactory().createDockableContainer(dockable, DockableContainerFactory.ParentType.PARENT_SPLIT_CONTAINER);
		dockableContainer.installDocking(this);
	...
	public void remove(Dockable dockable) {
		...
		if(dockLocation == DockableState.Location.DOCKED || dockLocation == DockableState.Location.FLOATING) { // ...2005/10/06
	...
	public void maximize(Dockable dockable) {
		...
		DockableState.Location currentLocation = getLocation(currentState);
		DockableStateWillChangeEvent dswe = new DockableStateWillChangeEvent(currentState, newState);
		DockingActionEvent dae = new DockingActionSimpleStateChangeEvent(this, dockable, currentLocation, DockableState.Location.MAXIMIZED);
		...
		Insets i = getDockingPanelInsets();
		if(DockingPreferences.isLightWeightUsageEnabled()) {
			maximizedComponent.setBounds(i.left, i.top, getWidth() - i.left - i.right, getHeight() - i.top - i.bottom);
			...
			if(DockingPreferences.isSingleHeavyWeightComponent()) {
				...
				if(DockingUtilities.isHeavyWeightComponent(comp)) {
					...
					awtPanel.setBounds(i.left, i.top, getWidth() - i.left - i.right, getHeight() - i.top - i.bottom);
	...
	public void restore(final Dockable dockable) {
		...
		DockableStateWillChangeEvent dswe = new DockableStateWillChangeEvent(currentState, newState);
		DockingActionEvent dae = new DockingActionSimpleStateChangeEvent(this, dockable, currentLocation, DockableState.Location.DOCKED);
		...
		if(dummyMaximedReplacer.getParent() instanceof TabbedDockableContainer) {
			sdc = DockableContainerFactory.getFactory().createDockableContainer(dockable, DockableContainerFactory.ParentType.PARENT_TABBED_CONTAINER);
		} else {
			sdc = DockableContainerFactory.getFactory().createDockableContainer(dockable, DockableContainerFactory.ParentType.PARENT_SPLIT_CONTAINER);
		}
	...
	public void setFloating(final Dockable dockable, boolean floating, Point screenPosition) {
		...
		if(floating) {
			...
			if(location == DockableState.Location.FLOATING) {
				...
				DockingActionEvent dae = new DockingActionSimpleStateChangeEvent(this, dockable, currentLocation, DockableState.Location.FLOATING);
				if(isDockingActionAccepted(dae, event)) {
			...
			try {
				if(parent instanceof SplitContainer) {
					...
					if(tparent.getTabCount() == 1) { // no more use for tabs
						...
						if(floating) {
							lastContainer = DockableContainerFactory.getFactory().createDockableContainer(last, DockableContainerFactory.ParentType.PARENT_DETACHED_WINDOW);
						} else {
							lastContainer = DockableContainerFactory.getFactory().createDockableContainer(last, DockableContainerFactory.ParentType.PARENT_SPLIT_CONTAINER);
						}
						lastContainer.installDocking(this);

						DockingUtilities.replaceChild(((Component) tparent).getParent(), (Component) tparent, (Component) lastContainer);
					}
				} else if(parent == dockingPanel) { // no more views to show
					dockingPanel.remove(0);
				} else if(SwingUtilities.getWindowAncestor(parent) instanceof FloatingDockableContainer) {
	...
	public void close(Dockable dockable) {
		...
		if(dockingPanel.isAncestorOf(dockable.getComponent())) {
			...
			if(isDockingActionAccepted(dae, dswe)) {
				...
				context.setDockableState(dockable, new DockableState(this, dockable, DockableState.Location.CLOSED, currentState.getPosition()));
				dockable.getDockKey().setLocation(DockableState.Location.CLOSED);
				fireDockingAction(dae, new DockableStateChangeEvent(currentState, newState));
			}
	...
	public void setAutoHide(Dockable dockable, boolean hide) {
		...
		if(hide) {
			...
			DockableStateWillChangeEvent dswe = new DockableStateWillChangeEvent(currentState, newState);
			DockingActionEvent dae = new DockingActionSimpleStateChangeEvent(this, dockable, currentLocation, DockableState.Location.HIDDEN);
		...
		for(int i = 0; i < dockables.getLength(); i++) {
			...
			if(fdc == null) {
				...
				if(tdc.getTabCount() == 0) {
					...
					DockingUtilities.replaceChild(((Component) base).getParent(), (Component) base, (Component) tdc);
					tdc.addDockable(baseDockable, 0);
				}
				tdc.addDockable(dockable, tdc.getTabCount());
			}

			context.setDockableState(dockable, new DockableState(this, dockable, DockableState.Location.FLOATING, position));
			dockable.getDockKey().setLocation(DockableState.Location.FLOATING);
	...
	public void addDockable(CompoundDockable base, Dockable dockable) {
		...
		DockingActionEvent dae = new DockingActionAddDockableEvent(this, dockable, currentLocation, newState.getLocation(), panel);

		if(isDockingActionAccepted(dae, dswe)) {
			DockableContainer dockableContainer = DockableContainerFactory.getFactory().createDockableContainer(dockable, DockableContainerFactory.ParentType.PARENT_DESKTOP);
			dockableContainer.installDocking(this);
